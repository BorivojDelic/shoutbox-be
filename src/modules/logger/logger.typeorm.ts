...
import { LOGGING_LEVELS } from '../../shared/constants/global.constants';
...
export class TypeORMLogger implements Logger {
  ...
  logQuery(query: string, parameters?: any[]): any {
    if (LOGGING_LEVELS.includes('query')) {
      this.logger.debug(`${query}, ${parameters?.join(', ')}`);
    }
  }
  ...
  logQueryError(error: string, query: string, parameters?: any[]): any {
    this.logger.error(`${error}, ${query}, ${parameters?.join(', ')}`);
  }
  ...
  logQuerySlow(time: number, query: string, parameters?: any[]): any {
    this.logger.info(`${time}, ${query}, ${parameters?.join(', ')}`);
  }
